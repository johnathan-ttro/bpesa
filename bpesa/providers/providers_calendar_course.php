...
class SimpleCalendar {
	...
	public function show( $echo = true ) {
		...
		$out = '<table cellpadding="0" cellspacing="0" class="SimpleCalendar"><thead>
				<tr>
					<th class="typeface-js" colspan="2"><a class="prev" id="next" href="'. $this->naviHref .'?month=' . sprintf('%02d',$preMonth) . '&year=' . $preYear . '#next">Prev</a></th>
					<th class="typeface-js" colspan="3"><span class="title">'. date('Y M', strtotime($this->currentYear . '-' . $this->currentMonth.'-1')) . '</span></th>
					<th class="typeface-js" colspan="2"><a class="next" id="previous" href="' . $this->naviHref . '?month=' . sprintf("%02d", $nextMonth) . '&year=' . $nextYear . '#previous">Next</a></th>
				</tr>
				<tr>';
		...
		for( $i = 1; $i <= $no_days; $i++ ) {
			...
			if($currentUrl == HOSTNAME . 'providers/providers_courses.php'){
		        $venueLink = HOSTNAME . 'providers/providers_add_venues.php';
		    }else{
		        $venueLink;
		    }

			$out .= '<time datetime="' . date('Y-m-d', $datetime) . '"><a href="' . $venueLink . '">'. $i .'</a></time>';
