...
class SimpleCalendar {
	...
	public function addDailyHtml( $html, $start_date_string, $end_date_string = false ) {
		...
		//echo $start_date = $start_date_string .'<br />';
		if( $end_date_string ) {
		...
		do {
			...
			//echo $tDate['year'] .'' .$tDate['mon'] . '' . $tDate['mday'] .'<br />';
	...
	public function show( $echo = true ) {
		...
		$out = '<table cellpadding="0" cellspacing="0" class="SimpleCalendar"><thead>
				<tr>
					<th colspan="2"><a class="prev" id="next" href="'. $this->naviHref.'?month='. sprintf('%02d',$preMonth) . '&year='.$preYear.'#next">Prev</a></th>
					<th colspan="3"><span class="title">'. date('Y M', strtotime($this->currentYear.'-' . $this->currentMonth.'-1')) . '</span></th>
					<th colspan="2"><a class="next" id="previous" href="'.$this->naviHref.'?month='.sprintf("%02d", $nextMonth).'&year='.$nextYear.'#previous">Next</a></th>
				</tr>
				<tr>';
		...
		for( $i = 1; $i <= $no_days; $i++ ) {
			...
			if($currentUrl == HOSTNAME . 'vendors/vendors_venues.php'){
		        $venueLink = HOSTNAME . 'vendors/vendors_add_venues.php';
		    }else{
		        $venueLink;
		    }

			$out .= '<time datetime="' . date('Y-m-d', $datetime) . '"><a href="' . $venueLink . '?date=' . date('d-m-Y', $datetime) . '">'. $i .'</a></time>';

			$dHtml_arr = false;

			//echo $this->now['year'] .' '. $this->now['mon'] . $i .'<br />' ;
